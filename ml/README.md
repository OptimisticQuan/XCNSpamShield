# XCNSpamShield ML Pipeline

This directory contains the preprocessing, training, evaluation, and export pipeline for the XCNSpamShield spam classifier.
...
- `convert-tfjs` uses the `export` extra directly. The current working chain is `onnx2tf + tensorflow + tensorflowjs`, plus the extra runtime helpers that `onnx2tf` needs at conversion time.
